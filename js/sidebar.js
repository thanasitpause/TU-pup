...
function addSidebarToggleButton() {
  ...
  const navbarLogo = document.querySelector('nav a.font-bold, nav a.flex');
  
  if (navbarLogo && !document.getElementById('sidebar-toggle')) {
    // สร้างปุ่ม Hamburger
    const toggleButton = document.createElement('button');
    toggleButton.id = 'sidebar-toggle';
    toggleButton.className = 'mr-3 text-gray-500 hover:text-tu-red-500 transition-colors focus:outline-none';
...
// ฟังก์ชัน Toggle Sidebar - แก้ไขปัญหาการปิดไม่ได้
...
// ฟังก์ชันปิด Sidebar - แก้ไขแล้ว
function closeSidebar() {
  const sidebar = document.getElementById('sidebar');
  const pageContent = document.getElementById('page-content');
  
  if (!sidebar.classList.contains('-translate-x-full')) {
    // ปิด sidebar โดยเพิ่ม class -translate-x-full ทันที
    sidebar.classList.add('-translate-x-full');
    
    // ลบ margin ซ้ายของ content
...
async function updateSidebarAuthUI() {
  try {
    ...
    if (!authenticatedSection || !guestSection) {
      return; // ป้องกันกรณี elements ไม่ถูกโหลด
    }
    ...
    if (user) {
      ...
      if (newPostMenuItem) newPostMenuItem.classList.remove('hidden');
      if (myPostsMenuItem) myPostsMenuItem.classList.remove('hidden');
      ...
      if (logoutBtn) {
        // ลบ event listener เดิมเพื่อป้องกันการทำงานซ้ำ
      ...
      if (newPostMenuItem) newPostMenuItem.classList.add('hidden');
      if (myPostsMenuItem) myPostsMenuItem.classList.add('hidden');
    }
  } catch (error) {
...
function setActiveLink() {
  ...
  sidebarLinks.forEach(link => {
    ...
    if (currentPath === linkPath || 
      ...
      // เพิ่ม Class สำหรับ Active Link - ใช้ Class ของธีม TU
      link.classList.add('bg-tu-red-50', 'text-tu-red-500', 'font-medium', 'sidebar-active-item');
      // เปลี่ยนสีไอคอน
      const icon = link.querySelector('svg');
      if (icon) {
        icon.classList.remove('text-gray-400');
        icon.classList.add('text-tu-red-500');
      }
    }
  });
}
